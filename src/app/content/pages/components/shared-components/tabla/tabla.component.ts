...
export class TablaComponent implements  OnChanges, OnInit, AfterViewInit{
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (this.datos) {
            ...
            this.totalesACalcular.forEach(total => {
                var indice = this.valorColumnas.indexOf(total)
                this.totales[indice] = this.datos.reduce(function(sum, value) {
                    return sum + value[total];
                }, 0);
            })
            this.promediosACalcular.forEach(prom => {
                var indice = this.valorColumnas.indexOf(prom)
                this.totales[indice] = this.datos.reduce(function(sum, value) {
                    return sum + value[prom];
                }, 0);
                var cantElemsPromedio = this.datos.filter(dato => (dato[prom] != null && dato[prom] != '')).length
    ...
    Filtrar(columna) {
        ...
        setTimeout(function() {
            ...
            let datosfinales = memphis.Filtrado(memphis.datos);
