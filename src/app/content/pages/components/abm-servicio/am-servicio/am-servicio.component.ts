...
@Component({
	...
	animations: [
		trigger('slowEnterRight', [
			transition(':enter', [
				useAnimation(animaciones.enterRightShort)
			]),
			transition(':leave', [
				useAnimation(animaciones.exitRightShort)
			]),
		]),
		trigger('slowEnterUp', [
			transition(':enter', [
				useAnimation(animaciones.enterUpShort)
			]),
			transition(':leave', [
				useAnimation(animaciones.exitUpShort)
			]),
		]),
	]
})
export class AmServicioComponent implements OnChanges {
	...
	dias = [
		...
		{ seleccionado: false, dia: 'miercoles', horarios: [new HorarioServicio] },
	...
	ngOnChanges(changes: SimpleChanges) {
		...
		if (this.editando) {
			this.dias = this.dias.map(d => {
				const { dia, horarios } = this.servicioAModificar.horarios.find(_horario => _horario.dia === d.dia);
				return (horarios) ? { seleccionado: true, dia, horarios } : d;
			});
		} else {
			this.dias = this.dias.map(({ dia }) =>
				({ seleccionado: false, dia: dia, horarios: [new HorarioServicio] })
			)
		}
	}
	...
	borrar() {
		...
		this.dias = this.dias.map((dia) => ({ seleccionado: false, dia: dia.dia, horarios: [new HorarioServicio] }));
	}
	...
	actualizarRestoHorario = (hora, valorAnterior) => {
		...
		if (hora) {
			...
			const duracionHoras = moment.duration(valorActual.diff(valorAnterior)).hours();
			const duracionMinutos = moment.duration(valorActual.diff(valorAnterior)).minutes();
			hora.horaFin = hora.horaFin && moment(hora.horaFin, 'HH:mm').add(duracionHoras, 'h').add(duracionMinutos, 'm').format('HH:mm')
			hora.entradaDesde = hora.entradaDesde && moment(hora.entradaDesde, 'HH:mm').add(duracionHoras, 'h').add(duracionMinutos, 'm').format('HH:mm')
			hora.entradaHasta = hora.entradaHasta && moment(hora.entradaHasta, 'HH:mm').add(duracionHoras, 'h').add(duracionMinutos, 'm').format('HH:mm')
	...
	agregarHorario = (dia) => dia.horarios.push(new HorarioServicio(dia.horarios[0].horaInicio, dia.horarios[0].horaFin, dia.horarios[0].entradaDesde, dia.horarios[0].entradaHasta))
