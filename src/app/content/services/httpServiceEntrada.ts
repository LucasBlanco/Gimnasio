...
export class HttpServiceEntrada {
  ...
  public acceder(idSocio) {
    ...
    this.httpService
      .post(
        new Modelos.Post("/socio/acceder", {
      ...
      .then((response: any) => {
        if (response === RECHAZO) {
          this.httpService.sendMessage(
            "El socio tiene la cuota impaga para el servicio. Dirijase a la compra para realizar la renovacion",
            "error"
          );
        } else if (response === ACEPTO) {
          this.httpService.sendMessage(
            "Se ha registrado la entrada",
            "success"
          );
        } else {
          this.httpService.sendMessage(
            "Hay servicios en conflicto para la hora actual. Se debera resolver manualmente",
            "warning"
          );
          ...
          const servicios = response.servicios.map(
            srv => new Servicio(srv.nombre, null, null, srv.id)
          );
  ...
  public registrarEntrada(socios: Socio[], servicios: Servicio[]) {
    ...
    return this.httpService
      .post(
        new Modelos.Post(
          "/servicio/registrarEntradas",
          { socios: idSocios, servicios: idServicios },
  ...
  public devolverEntradas(socios: Socio[], servicio: Servicio) {
    ...
    return this.httpService.post(
      new Modelos.Post(
        "/servicio/devolverEntradas",
        { idServicio: servicio.id, socios: idSocios },
